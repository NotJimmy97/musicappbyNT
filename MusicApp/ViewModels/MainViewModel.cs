...
namespace MusicApp.ViewModels
{
    public class MainViewModel : ObservableObject, IDisposable
    {
        ...
        public MainViewModel(IMusicApiClient apiClient, NowPlayingViewModel nowPlaying)
        {
            ...
            // Initial query to populate catalog for the user
        ...
        private void OnSearchKeywordChanged(string query)
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    // Debounce delay 400ms to suppress redundant requests during keystrokes
        ...
        private async Task ExecuteSearchAsync(string query, CancellationToken token)
        {
            if (string.IsNullOrWhiteSpace(query))
            {
                ...
                {
                    await dispatcher.InvokeAsync(() => SearchResults.Clear());
                }
                else
                {
                    SearchResults.Clear();
            ...
            try
            {
                ...
                if (dispatcher != null && !dispatcher.CheckAccess())
                {
                    await dispatcher.InvokeAsync(updateList);
                }
                else
                {
                    updateList();
